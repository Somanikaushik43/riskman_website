...
const HeroSection = () => {
    return (
        <section className="relative pt-16 md:pt-24 pb-16 md:pb-12 px-4 sm:px-6 md:px-10 lg:px-12 max-w-[1280px] mx-auto grid lg:grid-cols-12 gap-8 items-start min-h-screen bg-[#05051e] overflow-hidden">

          <BackgroundGrid/>

          {/* Decorative Background Icons - Multiple transparent icons scattered behind text */}
          <div className="absolute inset-0 z-10 pointer-events-none opacity-20">
            {/* Icon 1 - Top Right */}
            <div className="absolute w-32 h-32 border-2 rounded-full top-20 right-20 border-indigo-500/30"></div>
            
            {/* Icon 2 - Middle Left */}
            <div className="absolute w-40 h-40 border-2 rounded-full top-1/2 -left-10 border-blue-500/20"></div>
            
            {/* Icon 3 - Bottom Center */}
            <div className="absolute border-2 rounded-full bottom-32 left-1/4 w-28 h-28 border-purple-500/25"></div>
            
            {/* Icon 4 - Top Center */}
            <div className="absolute w-24 h-24 border-2 rounded-full top-32 left-1/3 border-indigo-400/20"></div>
            
            {/* Icon 5 - Right Side */}
            <div className="absolute border-2 rounded-full top-2/3 right-1/4 w-36 h-36 border-blue-400/15"></div>
            
            {/* Icon 6 - Far Right */}
            <div className="absolute w-32 h-32 border-2 rounded-full top-1/3 right-10 border-purple-400/20"></div>
          </div>
            ...
            <div className="relative z-20 flex flex-col w-full p-0 lg:col-span-7 lg:pl-10">
                ...
                {/* Top Left Floating Icon (Calculator) - Visible on all devices */}
                <div className="absolute -left-8 sm:left-0 -top-6 sm:-top-10 items-center justify-center w-10 sm:w-12 md:w-16 h-10 sm:h-12 md:h-16 bg-[#1a1a2e] rounded-2xl border border-white/10 shadow-2xl flex animate-bounce duration-[3000ms]">
                    <svg className="w-5 h-5 text-white sm:w-6 md:w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                    </svg>
                </div>

                {/* Floating Icon (Pie Chart) - Visible on all devices with responsive positioning */}
                <div className="absolute left-16 sm:left-24 md:left-40 lg:left-[450px] -top-4 sm:-top-6 md:-top-8 items-center justify-center w-10 sm:w-12 md:w-16 h-10 sm:h-12 md:h-16 bg-[#1a1a2e] rounded-2xl border border-white/10 shadow-2xl flex animate-pulse duration-[4000ms] z-20">
                    <svg className="w-5 h-5 text-white sm:w-6 md:w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-[6.5rem] font-bold text-white leading-tight md:leading-[1.05] mb-6 md:mb-12 tracking-tight">
                    Clarity, Risk <br />
                    <span className="text-white">Value</span> <br />
                    <span className="text-white">Assurance</span>
                </h1>

                <p className="max-w-2xl mb-8 text-sm font-light leading-relaxed text-gray-300 md:mb-12 sm:text-base md:text-lg">
                ...
                <div className="relative w-full mb-8 md:mb-12">
                    {/* Search Bar */}
                    <div className="bg-white rounded-full p-1.5 sm:p-2 flex items-center shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full relative z-20 h-12 sm:h-14 md:h-[4.5rem]">
                        <input
                            type="text"
                        ...
                        />
                        <button className="bg-[#5a5afc] hover:bg-[#4a4af0] text-white px-3 sm:px-6 md:px-8 py-2 md:py-3 rounded-full font-semibold transition-colors text-xs sm:text-sm md:text-lg h-full whitespace-nowrap shadow-md flex-shrink-0">
                            Search
                        </button>
                    </div>
                </div>
                ...
                <div className="relative z-30 flex flex-wrap items-center gap-2 mb-16 sm:gap-3 md:gap-4 md:mb-24">
                    ...
                    <div className="flex items-center justify-center w-12 sm:w-14 h-12 sm:h-14 bg-[#0a0a2e] rounded-2xl border border-white/10 shadow-lg shrink-0">
                        <span className="text-xl font-bold text-white sm:text-2xl">S</span>
                    </div>

                    {["Risk Assurance", "Systems Audit", "ESG Assurance"].map((tag) => (
                        <div key={tag} className="flex items-center gap-2 px-2 sm:px-4 md:px-6 py-2 md:py-3 bg-[#1a1a3a] border border-white/10 rounded-full text-gray-200 text-xs sm:text-sm cursor-pointer hover:bg-[#252545] transition-colors shadow-lg whitespace-nowrap">
            ...
            <div className="relative flex flex-col items-center lg:col-span-5">
                ...
                <div className="grid w-full grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-8 rounded-[1rem] text-slate-900 flex flex-col justify-between min-h-[250px] shadow-2xl hover:-translate-y-2 transition-transform duration-500">
                        <div className="p-3 text-blue-600 bg-slate-100 rounded-xl w-fit">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
